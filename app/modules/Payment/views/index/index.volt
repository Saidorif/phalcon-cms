	...
	<div class="container">
		<div class="main-content">
			...
			<div class="invoice-wrap">
				<div class="row">
					<div class="col-sm-8 col-sm-offset-2 success-pay">
							<h2 class="text-center mb-30 text-uppercase">
								{{helper.translate('Join the association')}}
							</h2>
						<div class="row">
							<div class="col-sm-6">
								<h5>{{helper.translate('name')}}</h5>
								<h5>{{ helper.translate('Address of company') }}</h5>
								<h5>{{ helper.translate('Sphere') }}</h5>
								<h5>{{ helper.translate('Status of company') }}</h5>
								<h5>{{helper.translate('Phone')}}</h5>
								<h5>{{helper.translate('Email')}}</h5>
							</div>
							<div class="col-sm-6 simple-text mb-30">
								{% if order %}
									...
									<h5>{{order.address}}</h5>
									<h5>{{order.sphere}}</h5>
									<h5>{{helper.translate(order.company_status)}}</h5>
									<h5>{{order.phone}}</h5>
									<h5>{{order.email}}</h5>								
								{% endif %}
							</div>
			<form method="POST" action="https://checkout.paycom.uz/" class="m-auto">
			    ...
			    <button type="submit" class="blue upp payBtn">
			    	{{helper.translate('Pay')}}
			    </button>
			</form>							
						</div><!-- /. row -->
					</div><!-- /. column -->
