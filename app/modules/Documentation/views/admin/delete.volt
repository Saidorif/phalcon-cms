<div class="ui segment">
    <a href="{{ url.get() }}documentation/admin/edit/{{ model.getId() }}?lang={{ constant('LANG') }}" class="ui button">
        <i class="icon left arrow"></i> {{ helper.at('Back') }}
    </a>

    <form method="post" class="ui negative message form" action="">
        <p>{{ helper.at('Delete documentation') }} <b>{{ model.getId() }} | {{ model.getTitle() }}</b>?</p>
